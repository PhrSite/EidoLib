/*
 * NiemTypes
 *
 * NIEM Types for the EIDO
 *
 * OpenAPI spec version: 1.0.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */
using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;

namespace NiemTypes
{ 
    /// <summary>
    /// A data type for a distortion in an image
    /// </summary>
    [DataContract]
    public partial class BiomImageDistortionType : IEquatable<BiomImageDistortionType>
    { 
        /// <summary>
        /// A kind of distortion in an image NIEM reference is biom:ImageDistortionCategoryCode
        /// </summary>
        /// <value>A kind of distortion in an image NIEM reference is biom:ImageDistortionCategoryCode</value>

        [DataMember(Name="categoryCode")]
        public List<BiomImageDistortionCategoryCodeType> CategoryCode { get; set; }

        /// <summary>
        /// A manner in which distortion in an image was measured, i.e. whether it was estimated or calculated NIEM reference is biom:ImageDistortionMeasurementCode
        /// </summary>
        /// <value>A manner in which distortion in an image was measured, i.e. whether it was estimated or calculated NIEM reference is biom:ImageDistortionMeasurementCode</value>

        [DataMember(Name="measurementCode")]
        public List<BiomImageDistortionMeasurementCodeType> MeasurementCode { get; set; }

        /// <summary>
        /// A severity of distortion in an image NIEM reference is biom:ImageDistortionSeverityCode
        /// </summary>
        /// <value>A severity of distortion in an image NIEM reference is biom:ImageDistortionSeverityCode</value>

        [DataMember(Name="severityCode")]
        public List<BiomImageDistortionSeverityCodeType> SeverityCode { get; set; }

        /// <summary>
        /// biomImageDistortionType JSON-LD context
        /// </summary>
        /// <value>biomImageDistortionType JSON-LD context</value>
        [JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public enum ContextEnum
        {
            /// <summary>
            /// Enum JSONLDContextsbiomImageDistortionTypeContextJsonldEnum for ../JSON-LD_Contexts/biomImageDistortionTypeContext.jsonld
            /// </summary>
            [EnumMember(Value = "../JSON-LD_Contexts/biomImageDistortionTypeContext.jsonld")]
            JSONLDContextsbiomImageDistortionTypeContextJsonldEnum = 0        }

        /// <summary>
        /// biomImageDistortionType JSON-LD context
        /// </summary>
        /// <value>biomImageDistortionType JSON-LD context</value>

        [DataMember(Name="@context")]
        public ContextEnum? Context { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class BiomImageDistortionType {\n");
            sb.Append("  CategoryCode: ").Append(CategoryCode).Append("\n");
            sb.Append("  MeasurementCode: ").Append(MeasurementCode).Append("\n");
            sb.Append("  SeverityCode: ").Append(SeverityCode).Append("\n");
            sb.Append("  Context: ").Append(Context).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((BiomImageDistortionType)obj);
        }

        /// <summary>
        /// Returns true if BiomImageDistortionType instances are equal
        /// </summary>
        /// <param name="other">Instance of BiomImageDistortionType to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(BiomImageDistortionType other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    CategoryCode == other.CategoryCode ||
                    CategoryCode != null &&
                    CategoryCode.SequenceEqual(other.CategoryCode)
                ) && 
                (
                    MeasurementCode == other.MeasurementCode ||
                    MeasurementCode != null &&
                    MeasurementCode.SequenceEqual(other.MeasurementCode)
                ) && 
                (
                    SeverityCode == other.SeverityCode ||
                    SeverityCode != null &&
                    SeverityCode.SequenceEqual(other.SeverityCode)
                ) && 
                (
                    Context == other.Context ||
                    Context != null &&
                    Context.Equals(other.Context)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (CategoryCode != null)
                    hashCode = hashCode * 59 + CategoryCode.GetHashCode();
                    if (MeasurementCode != null)
                    hashCode = hashCode * 59 + MeasurementCode.GetHashCode();
                    if (SeverityCode != null)
                    hashCode = hashCode * 59 + SeverityCode.GetHashCode();
                    if (Context != null)
                    hashCode = hashCode * 59 + Context.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(BiomImageDistortionType left, BiomImageDistortionType right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(BiomImageDistortionType left, BiomImageDistortionType right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
