/*
 * NiemTypes
 *
 * NIEM Types for the EIDO
 *
 * OpenAPI spec version: 1.0.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */
using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;

namespace NiemTypes
{ 
    /// <summary>
    /// A data type for a number value.
    /// </summary>
    [DataContract]
    public partial class NcNumericType : NiemXsdecimal, IEquatable<NcNumericType>
    { 
        /// <summary>
        /// A value that indicates belief that content is accurate or trustworthy. NIEM reference is nc:confidencePercent
        /// </summary>
        /// <value>A value that indicates belief that content is accurate or trustworthy. NIEM reference is nc:confidencePercent</value>

        [DataMember(Name="confidencePercent")]
        public decimal? ConfidencePercent { get; set; }

        /// <summary>
        /// An allowable range for error on a point estimate. NIEM reference is nc:toleranceNumeric
        /// </summary>
        /// <value>An allowable range for error on a point estimate. NIEM reference is nc:toleranceNumeric</value>

        [DataMember(Name="tolerance")]
        public decimal? Tolerance { get; set; }

        /// <summary>
        /// ncNumericType JSON-LD context
        /// </summary>
        /// <value>ncNumericType JSON-LD context</value>
        [JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
        public enum ContextEnum
        {
            /// <summary>
            /// Enum JSONLDContextsncNumericTypeContextJsonldEnum for ../JSON-LD_Contexts/ncNumericTypeContext.jsonld
            /// </summary>
            [EnumMember(Value = "../JSON-LD_Contexts/ncNumericTypeContext.jsonld")]
            JSONLDContextsncNumericTypeContextJsonldEnum = 0        }

        /// <summary>
        /// ncNumericType JSON-LD context
        /// </summary>
        /// <value>ncNumericType JSON-LD context</value>

        [DataMember(Name="@context")]
        public ContextEnum? Context { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class NcNumericType {\n");
            sb.Append("  ConfidencePercent: ").Append(ConfidencePercent).Append("\n");
            sb.Append("  Tolerance: ").Append(Tolerance).Append("\n");
            sb.Append("  Context: ").Append(Context).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public  new string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((NcNumericType)obj);
        }

        /// <summary>
        /// Returns true if NcNumericType instances are equal
        /// </summary>
        /// <param name="other">Instance of NcNumericType to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(NcNumericType other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    ConfidencePercent == other.ConfidencePercent ||
                    ConfidencePercent != null &&
                    ConfidencePercent.Equals(other.ConfidencePercent)
                ) && 
                (
                    Tolerance == other.Tolerance ||
                    Tolerance != null &&
                    Tolerance.Equals(other.Tolerance)
                ) && 
                (
                    Context == other.Context ||
                    Context != null &&
                    Context.Equals(other.Context)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (ConfidencePercent != null)
                    hashCode = hashCode * 59 + ConfidencePercent.GetHashCode();
                    if (Tolerance != null)
                    hashCode = hashCode * 59 + Tolerance.GetHashCode();
                    if (Context != null)
                    hashCode = hashCode * 59 + Context.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(NcNumericType left, NcNumericType right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(NcNumericType left, NcNumericType right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
